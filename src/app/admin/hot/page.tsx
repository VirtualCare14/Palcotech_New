...
export default async function AdminHotPage() {
  await connectToDatabase();
  const products = await ProductModel.find(
    {},
    { name: 1, slug: 1, isHot: 1, updatedAt: 1 },
  )
    .sort({ updatedAt: -1 })
    .limit(80)
    .lean();
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">Hot Products</h1>
            <p className="mt-1 text-sm text-slate-600">
              Toggle products that should appear in the homepage “Hot products” section.
            </p>
          </div>
                    ...
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-xs font-semibold shadow-sm transition ${
                        p.isHot
                          ...
                          : "border border-slate-200 bg-white text-slate-900 hover:border-rose-200 hover:bg-rose-50"
                      }`}
                    >
                      {p.isHot ? "Hot" : "Mark as Hot"}
                    </button>
                  </form>
                </div>
              ))
